...
/** @type {string} */
const CACHE_VERSION = '1788320905|2779297';
/** @type {string} */
...
self.addEventListener('install', (event) => {
	event.waitUntil(caches.open(CACHE_NAME).then((cache) => Promise.all(CACHED_FILES.map((f) => fetch(f, { cache: 'no-cache' }).then((r) => cache.put(f, r))))));
});
